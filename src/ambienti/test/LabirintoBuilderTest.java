...
public class LabirintoBuilderTest {
	...
	public void testMonolocaleConAttrezzoSingoloDuplicato() {
		...
		assertTrue(size==1);
		//non va perché le mie stanze hanno un set di attrezzi, non una lista
...
//		assertEquals(bilocale.getStanzaVincente(),bilocale.getStanzaIniziale().getStanzaAdiacente("nord"));
		assertEquals(bilocale.getStanzaVincente(),bilocale.getStanzaIniziale().getStanzaAdiacente(Direzione.valueOf("nord")));
		//non vanno esclusivamente perché io ho set di stanza e non lista di stanze
...
//		assertEquals("biblioteca",trilocale.getStanzaIniziale().getStanzaAdiacente("sud").getNome());
		assertEquals("biblioteca",trilocale.getStanzaIniziale().getStanzaAdiacente(Direzione.valueOf("sud")).getNome());
	}
	...
	public void testPiuDiQuattroAdiacenti() {
		Labirinto maze = labirintoBuilder
				...
				//riga sotto: come dovrebbe, da errore
...
//		assertNull(maze.getStanzaIniziale().getStanzaAdiacente("nord-est"));
		assertNull(maze.getStanzaIniziale().getStanzaAdiacente(Direzione.valueOf("nord-est")));
...
//		assertEquals(stanzaVincente,labirintoBuilder.getMappaStanze().get("stanza bloccata").getStanzaAdiacente("nord"));	
		assertEquals(stanzaVincente,labirintoBuilder.getMappaStanze().get("stanza bloccata").getStanzaAdiacente(Direzione.valueOf("nord")));
	}
...
//		assertEquals(stanzaBloccata,labirintoBuilder.getMappaStanze().get("stanza bloccata").getStanzaAdiacente("nord"));
		assertEquals(stanzaBloccata,labirintoBuilder.getMappaStanze().get("stanza bloccata").getStanzaAdiacente(Direzione.valueOf("nord")));
	}
...
//		Stanza corridoio = labirintoCompleto.getStanzaIniziale().getStanzaAdiacente("nord");
		Stanza corridoio = labirintoCompleto.getStanzaIniziale().getStanzaAdiacente(Direzione.valueOf("nord"));
		...
		//ho set di attrezzi, non lista...
		//assertEquals(Arrays.asList(a1,a2),corridoio.getAttrezzi());
		assertTrue(corridoio.getAttrezzi().size()==2);
	}
}
